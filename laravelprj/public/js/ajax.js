$(".ajaxBtn").click(function () {
  ...
  $.ajax({
    ...
    .then((res) => {
      ...
      if(resLength != 0){
          schedule.push('<h1>' + res[0][0].date_key + 'の予定</h1><input type="hidden" name="ajaxDateKey" value="'+ schedule_date_key +'">');
...
$(document).on("click", ".ajaxDelete", function(){
  ...
  $.ajax({
    ...
    .then((res) => {
      ...
      console.log(resLength);
      if(resLength != 0){
          schedule.push('<h1>' + res[0][0].date_key + 'の予定</h1>');
        ...
        var year = dateKeyForDelete.slice(0, 4);
        var month = dateKeyForDelete.slice(4, 6);
        var day = dateKeyForDelete.slice(6, 8);
        var yearText = '年';
        var monthText = '月';
        schedule_date_text = year + yearText + month + monthText + day + '日';
         schedule.push('<h1>' + schedule_date_text + 'の予定</h1>');
