...
if (isset($_POST["reg"])) {
	...
	if ($url == "") {
		$error_ay["url"] = "<span class='console'>選択してください</span>";
	}
...
if(!empty($id)){
    //編集の場合
    $sql = "SELECT * FROM `outputs` WHERE id = $id ORDER BY `outputs`.`create_date` ASC LIMIT 0 , 30";
    $rst = $mysqli->query($sql);
    if (!$rst) {
        echo 'system error.' . $mysqli->error;
        exit(1);
    ...
    //新規の場合
    $sql = "SELECT * FROM `outputs` ORDER BY `outputs`.`create_date` ASC LIMIT 0 , 30";
    $rst = $mysqli->query($sql);
    if (!$rst) {
    echo 'system error.' . $mysqli->error;
    exit(1);
        ...
        <form action="#pan" method="post" enctype="multipart/form-data">
            ...
            <div class="input">
                ...
                <textarea name="comment" cols="50" rows="5"><?= $comment ?></textarea><br /><?= $error_ay["comment"] ?>
						...
						<div class="input">
								<p>リンク</p>
								<input type="text" name="url" size="50" value="<?= $url ?>" /><br /><?= $error_ay["url"] ?>
						</div>
            <div class="btn">
